...
TEST_CASE("Basic tokenizer tests", "[tokenizer]") {
    CHECK(tokenEquality(tokenize("[", false), Token(Token::Type::LeftBracket, "[")));
    CHECK(tokenEquality(tokenize("]", false), Token(Token::Type::RightBracket, "]")));
    CHECK(tokenEquality(tokenize("{", false), Token(Token::Type::LeftBrace, "{")));
    CHECK(tokenEquality(tokenize("}", false), Token(Token::Type::RightBrace, "}")));
    CHECK(tokenEquality(tokenize(":", false), Token(Token::Type::Colon, ":")));
    CHECK(tokenEquality(tokenize("  ,", false), Token(Token::Type::Comma, ",", Position(2, 1, 3))));
    CHECK(tokenEquality(tokenize("true", false), Token(Token::Type::True, "true")));
    CHECK(tokenEquality(tokenize("   true", false), Token(Token::Type::True, "true", Position(3, 1, 4))));
    CHECK(tokenEquality(tokenize("false", false), Token(Token::Type::False, "false")));
    CHECK(tokenEquality(tokenize("\nfalse", false), Token(Token::Type::False, "false", Position(1, 2, 1))));
    CHECK(tokenEquality(tokenize("null", false), Token(Token::Type::Null, "null")));
    CHECK(tokenEquality(tokenize("  null:", false), Token(Token::Type::Null, "null", Position(2, 1, 3))));
}

TEST_CASE("String tokenizer tests", "[tokenizer]") {
    Token::Type t = Token::Type::String;

    CHECK(tokenEquality(tokenize("\"\"", false), Token(t, "")));
    CHECK(tokenEquality(tokenize("\"tralala\"", false), Token(t, "tralala")));
    CHECK(tokenEquality(tokenize("\"\\n\"", false), Token(t, "\n")));
    CHECK(tokenEquality(tokenize("\"\\t\"", false), Token(t, "\t")));
    CHECK(tokenEquality(tokenize("\" \"", false), Token(t, " ")));
